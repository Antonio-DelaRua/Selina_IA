...
def chat_with_bot(prompt, update_callback, finish_callback):
    ...
    try:
        ...
        update_callback("Llama a tu tio , que no se esta cargando la api")
        finish_callback()
...
def on_muneco_double_click(event):
    ...
    send_button = Button(frame, text="Enviar", command=send_message, bg='orange', fg='white', font=("Comic Sans MS", 12))
